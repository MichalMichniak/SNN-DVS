...
def call_spiking(tj, W, D_i, t_min, t_max, noise, dtype=torch.FloatTensor, device='cpu'):
    ...
    square_V_i_tmax = torch.sum((ti < t_max)*((torch.matmul((t_min - tj).type(dtype).to(device), W.type(dtype).to(device)))*(torch.matmul((t_min - tj).type(dtype).to(device), W.type(dtype).to(device)))))

    # Add noise to the spiking time for noise simulations
    if noise > 0:
        ti = ti + torch.randn_like(ti) * noise
    
    return ti, count_impulses, square_V_i_tmax
...
class SpikingDense(nn.Module):
    ...
    def forward(self, tj):
        ...
        if self.bias:
            ...
            output, count_impulses, square_V_i_tmax = call_spiking(new_tj, self.kernel, self.D_i, self.t_min, self.t_max, noise=self.noise, device=self.device)
        else:
            output, count_impulses, square_V_i_tmax = call_spiking(tj, self.kernel, self.D_i, self.t_min, self.t_max, noise=self.noise, device=self.device)
        ...
        return output, count_impulses, square_V_i_tmax
...
class SpikingConv2D(nn.Module):
    ...
    def set_params(self, t_min_prev, t_min, in_ranges_max, minimal_t_max = 0, in_scalar = 1):
        ...
        if self.B is not None:
            ...
            max_V = F.relu(torch.max(torch.sum(torch.mul(max_input,max_W),(1,2,3))+torch.maximum(self.B.squeeze(1), torch.zeros(self.B.squeeze(1).shape).to(self.device))/self.multiplier))#+eps_V/(self.multiplier)
        else:
            max_V = F.relu(torch.max(torch.sum(torch.mul(max_input,max_W),(1,2,3))))
            max_values = F.relu(torch.sum(torch.mul(max_input,max_W),(1,2,3)))
        self.t_min_prev = torch.tensor(t_min_prev, dtype=torch.float64, requires_grad=False)
    ...
    def call_spiking(self, tj, W, D_i, t_min, t_max, noise):
        ...
        count_impulses = torch.sum(ti < t_max)
        square_V_i_tmax = torch.sum((ti < t_max)*(torch.matmul((t_min - tj), W)*torch.matmul((t_min - tj), W)))
        return ti, count_impulses, square_V_i_tmax
    ...
    def call_spiking(self, tj, W, D_i, t_min, t_max, noise):
        ...
        #### Check ####
        V = torch.matmul((tj - t_min), torch.maximum(W, torch.zeros(W.shape).to(self.device)))
        if((V>threshold).any()):
            print(f"ERROR SpikingConv2D V {V}, thr {threshold}") 
        ### END Check ###
    ...
    def forward(self, tj):
        ...
        if (self.padding == 'valid' or self.BN != 1 or self.BN_before_ReLU == 1) and (self.B is None): 
            ...
            ti, count_impulses, square_V_i_tmax = self.call_spiking(tj, W * self.kernel_mul, self.D_i[0], self.t_min, self.t_max, noise=self.noise).transpose(1, 2)
            ...
            ti, count_impulses, square_V_i_tmax = self.call_spiking(tj, W, self.D_i[0], self.t_min, self.t_max, noise=self.noise)
            ti= ti.transpose(1, 2)
            if self.padding == 'valid':
                ti = ti.view(batch_size, out_channels, output_height, output_width)
            else:
...
def fuse_conv_and_bn(conv, bn, device = 'cuda:0'):
    ...
    if conv.bias is not None:
        ...
        b_conv = nn.Parameter(torch.rand( conv.weight.size(0) , dtype=torch.float32)*1e-10).to(device)

    b_bn = bn.bias - bn.weight.mul(bn.running_mean).div(torch.sqrt(bn.running_var + bn.eps))
    
    with torch.no_grad():
        fusedconv.bias.copy_( (torch.matmul(w_bn.to(device), nn.Parameter(b_conv).to(device)) + b_bn.to(device)) )
...
class AddSNNLayer(nn.Module):
    ...
    def forward(self, tj1, tj2):
        ...
        square_V_i_tmax = torch.sum((ti >= self.t_max)*(V*V))
        return ti, count_impulses, square_V_i_tmax
...
class SpikingConv2D_RELU1(nn.Module):
    def __init__(self, filters, name, X_n=1, padding='same', kernel_size=(3,3), robustness_params=None, kernels = None, device = 'cuda:0', biases = None, stride=1):
        super(SpikingConv2D_RELU1, self).__init__()
        if robustness_params is None:
            robustness_params = {}
        kernels_pos = kernels
        if biases is not None:
        ...
        kernels_new = kernels_pos.detach().clone()
        biases_new = biases_pos.detach().clone()
        ...
        kernels_new2 = kernels_pos.detach().clone()
        biases_new2 = (biases_pos.detach().clone() - 1)
        self.conv_second = SpikingConv2D(filters, name, device = device, padding=padding, stride=stride, kernel_size=kernel_size,robustness_params=robustness_params, kernels=kernels_new2, biases=biases_new2)
    ...
    def forward(self, tj):
        ...
        """
        #print(tj.shape)
        tj1, count_impulses1, square_V_i_tmax1 = self.conv_first(tj)
        tj2, count_impulses2, square_V_i_tmax2 = self.conv_second(tj)
        tj_sub, count_impulses3, square_V_i_tmax3 = self.sub(tj1, tj2)

        return tj_sub, count_impulses1+count_impulses2+count_impulses3, square_V_i_tmax1+square_V_i_tmax2+square_V_i_tmax3
        # return tj1

class AddSNNLayer_ReLU(nn.Module):
    """
    """
    def __init__(self, bias=0, device = 'cpu'):
        super(AddSNNLayer_ReLU, self).__init__()
        self.noise = 0
        self.B_n = 1 # Must be positive
    ...
    def forward(self, tj1, tj2):
        ...
        if(len(tj1.shape) == 3):
            if((torch.amax(self.t_min - tj1, dim=(1, 2))>(self.input1_val+eps_V)).any()):
                print(f"WARNING AddSNNLayer_ReLU input times not in declared range mismach: {(torch.amax(self.t_min - tj1, dim=(1, 2))-self.input1_val).max()}")
            self.channels = tj1.shape[0]
        elif(len(tj1.shape) == 4):
            if((torch.amax(self.t_min - tj1, dim=(2, 3))>(self.input1_val+eps_V)).any()):
                print(f"WARNING AddNNLayer_ReLU input times not in declared range mismach: {(torch.amax(self.t_min - tj1, dim=(2, 3))-self.input1_val).max()}")
            self.channels = tj1.shape[1]
        tj1_temp = tj1[0, :self.channels].detach().clone()

        tj3_temp = tj2[0, :self.channels].detach().clone()


        stacked = torch.stack([tj1_temp, tj3_temp], dim=0)
        ...
        min_times_list.append(self.t_min*torch.ones(min_times_list[0].shape).to(self.device))

        min_times = torch.stack(min_times_list, dim=0)
        mask_list = torch.stack(mask_list, dim=0)
        vect = torch.tensor([self.mul1, self.mul2]).to(self.device)
        ...
        for i in range(2):
            ...
            if((V_plus>threshold).any()):
                print(f"ERROR AddSNNLayer_ReLU V {V_plus.max()}, thr {threshold}")
        V_plus += ( 0 if self.t_min_prev==self.t_min else ((self.B/(self.multiplier * (self.t_min-self.t_min_prev)))* (self.t_min-min_times[i+1])) )
        if((V_plus>threshold).any()):
                print(f"ERROR AddSNNLayer_ReLU V {V_plus.max()}, thr {threshold}")
        V2 = (self.t_min - tj1[0, :self.channels])*self.mul1 + (self.t_min - tj2[0, :self.channels])*self.mul2 + self.B/(self.multiplier)*(1)
        if ((V2 - V_plus).abs().max() > 0.00001):
            print(f"WARNING AddSNNLayer_ReLU too big difference in chceck: {(V2 - V_plus).abs().max()}, {self.B}")

        #### END Check ####
        ti = (tj1[0, :self.channels] - self.t_min)*self.mul1 + (tj2[0, :self.channels] - self.t_min)*self.mul2 - self.B/(self.multiplier)*(1) + threshold + self.t_min
        ...
        if self.noise > 0:
            ti += torch.randn_like(ti) * self.noise
        count_impulses = torch.sum(ti < self.t_max)
        square_V_i_tmax = torch.sum((ti >= self.t_max)*(V2*V2))
        return ti, count_impulses, square_V_i_tmax
...
class AddSNNLayer_Htanh(nn.Module):
    def __init__(self, device='cpu'):
        ...
        self.noise = 0
        self.B_n = 1
        self.first = AddSNNLayer_ReLU(device=device)
        self.second = AddSNNLayer_ReLU(-1,device=device)
        self.sub = SubSNNLayer()
        pass
    ...
    def set_params(self, t_min_prev, t_min, input1_val, input2_val, minimal_t_max = 0, in_scalar1 = 1, in_scalar2 = 1):
        ...
        self.sub.t_max = max((1.0/in_scalar_sub)+tmins+eps_V, minimal_t_max)#tmaxs

        return tmins, self.sub.t_max, torch.minimum(sub_val,(1.0/in_scalar_sub)), in_scalar_sub

    def forward(self, tj1, tj2):
        tj_first, count_impulses1, square_V_i_tmax1 = self.first(tj1, tj2)
        tj_second, count_impulses2, square_V_i_tmax2 = self.second(tj1, tj2)
        tj_sub, count_impulses3, square_V_i_tmax3 = self.sub(tj_first, tj_second)
        return tj_sub, count_impulses1+count_impulses2+count_impulses3, square_V_i_tmax1+square_V_i_tmax2+square_V_i_tmax3
...
class SpikingConv2D_all(nn.Module):
    def __init__(self, filters, name, X_n=1, padding='same', kernel_size=(3,3), robustness_params=None, kernels = None, device = 'cuda:0', biases = None, stride=1):
        ...
        if robustness_params is None:
            robustness_params = {}
        kernels_pos = kernels.detach().clone()
        if biases is not None:
            biases_pos = biases.detach().clone()
        else:
            biases_pos = None
    ...
    def forward(self, tj):
        ...
        """
        tj1, count_impulses1, square_V_i_tmax1 = self.conv_first(tj)
        return tj1, count_impulses1, square_V_i_tmax1
        # return tj1

class ResidualSNNBlock_RELU1(nn.Module):
    def __init__(self,resblock : ResidualBlock, in_channels, out_channels, stride=1, downsample=None, robustness_params = None, device = "cuda:0", end_maxpool = False):
        super(ResidualSNNBlock_RELU1, self).__init__()
        ...
        if (downsample is not None):
            ...
            self.conv1 = SpikingConv2D_RELU1(out_channels, "temp1", device=device, padding=(1,1), stride=stride, kernel_size=(3,3),robustness_params=robustness_params, kernels=conv_fused.weight.data, biases= conv_fused.bias.data)
        self.device = device
        ...
        bn.eval()
        conv_fused = fuse_conv_and_bn(conv, bn)
        self.conv2 = SpikingConv2D_RELU1(out_channels, "temp1", device=device, padding=(1,1), stride=stride, kernel_size=(3,3),robustness_params=robustness_params, kernels=conv_fused.weight.data, biases= conv_fused.bias.data)

        self.downsample = downsample
        self.identity = IdentitySNNLayer()
        if end_maxpool:
            self.add_layer = AddSNNLayer_ReLU(device=device)
        else:
    ...
    def forward(self, x):
        ...
        square_V_i_tmax1, square_V_i_tmax2, square_V_i_tmax3, square_V_i_tmax4, square_V_i_tmax5 = 0,0,0,0,0
        if len(x.shape) == 3:
            x = x.unsqueeze(0)
        out, count_impulses1, square_V_i_tmax1 = self.conv1(x)
        if self.downsample:
            residual, count_impulses2, square_V_i_tmax2 = self.downsample(x)
            residual, count_impulses3 = self.pooling2(residual)
            out, count_impulses4 = self.pooling1(out)
            residual = residual
            out = out
        else:
            residual, count_impulses5, square_V_i_tmax3 = self.identity(x)
        out, count_impulses6, square_V_i_tmax4 = self.conv2(out)
        out, count_impulses7, square_V_i_tmax5 = self.add_layer(out,residual) # no need for adding negative part

        return out, count_impulses1+count_impulses2+count_impulses3+count_impulses4+count_impulses5+count_impulses6+count_impulses7, square_V_i_tmax1+square_V_i_tmax2+square_V_i_tmax3+square_V_i_tmax4+square_V_i_tmax5
    
class LayerSNN_ReLU1(nn.Module):
    def __init__(self, layer, inplanes, planes, blocks, stride=1, device = 'cuda:0', end_maxpool = False):
        super(LayerSNN_ReLU1,self).__init__()
        ...
        self.layers = []
        self.layers.append(ResidualSNNBlock_RELU1(layer[0],self.inplanes,planes, 1, downsample=downsample, device=device))
        self.inplanes = planes
        self.blocks = blocks
        for i in range(1, blocks):

            if i == blocks-1 and end_maxpool:
                self.layers.append(ResidualSNNBlock_RELU1(layer[i],self.inplanes,planes, 1, downsample=None, device=device, end_maxpool = True))
            else:
                self.layers.append(ResidualSNNBlock_RELU1(layer[i],self.inplanes,planes, 1, downsample=None, device=device))
    ...
    def forward(self, x):
        count_impulses_sum, square_V_i_tmax_sum = 0,0
        for i in range(self.blocks):

            x, count_impulses, square_V_i_tmax = self.layers[i].forward(x)
            count_impulses_sum += count_impulses
            square_V_i_tmax_sum += square_V_i_tmax
        return x, count_impulses_sum, square_V_i_tmax_sum
    
class SpikingDense_positive_ReLU1(nn.Module):
    """
    ...
    def __init__(self, units, input_units, name, weights, biases, X_n=1, outputLayer=False, robustness_params={}, input_dim=None,
        ...
        super().__init__()
        self.device = device
        W = weights.T.to(self.device)
        b1 = biases.to(self.device)
        b2 = (biases.detach().clone() - 1).to(self.device)
        self.first = SpikingDense(units,"test",robustness_params=robustness_params, device=self.device)
        self.second = SpikingDense(units,"test",robustness_params=robustness_params, device=self.device)
    ...
    def forward(self, tj):
        ...
        """
        # Call the custom spiking logic
        out1, count_impulses1, square_V_i_tmax1 = self.first(tj)
        out2, count_impulses2, square_V_i_tmax2 = self.second(tj)
        sub_, count_impulses3, square_V_i_tmax3 = self.sub_layer(out1,out2)
        return sub_, count_impulses1+count_impulses2+count_impulses3, square_V_i_tmax1+square_V_i_tmax2+square_V_i_tmax3
    
class SpikingDense_pos_all(nn.Module):
    """
    ...
    def __init__(self, units, input_units, name, weights, biases, X_n=1, outputLayer=False, robustness_params={}, input_dim=None,
        ...
        super().__init__()
        self.device=device
        W1 = weights.T.to(self.device)
        W2 = -weights.T.to(self.device)
        W = torch.concatenate((W1,W2),dim=1).to(self.device)
    ...
    def forward(self, tj):
        ...
        # Call the custom spiking logic
        out1, count_impulses1, square_V_i_tmax1 = self.first(tj)
        return out1, count_impulses1, square_V_i_tmax1
