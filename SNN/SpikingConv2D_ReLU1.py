...
class AddSNNLayer_ReLU(nn.Module):
    ...
    def forward(self, tj1, tj2):
        ...
        min_times_list.append(self.t_min*torch.ones(min_times_list[0].shape))

        min_times = torch.stack(min_times_list, dim=0)
        mask_list = torch.stack(mask_list, dim=0)
        vect = torch.tensor([self.mul1, self.mul2])
        V_plus = torch.zeros(min_times[0].shape) + (0 if self.t_min_prev==self.t_min else ((self.B/(self.multiplier * (self.t_min-self.t_min_prev)))* (min_times[0]-self.t_min_prev)))
        for i in range(2):
        ...
        # print("tttt",(self.t_min - tj1[0, :self.channels]).max())
        # print((self.t_min - tj2[0, :self.channels]).max())
        # print("tttt",(self.t_min - tj1[0, :self.channels]).max()*self.mul1)
        # print((self.t_min - tj2[0, :self.channels]).max()*self.mul2)
        # print(V2.max())
...
class AddSNNLayer_Htanh(nn.Module):
    def __init__(self):
        ...
        self.noise = 0
        self.B_n = 1
        self.first = AddSNNLayer_ReLU()
        self.second = AddSNNLayer_ReLU(1)
        self.sub = SubSNNLayer()
        pass
...
class ResidualSNNBlock_RELU1(nn.Module):
    def __init__(self,resblock : ResidualBlock, in_channels, out_channels, stride=1, downsample=None, robustness_params = None, device = "cuda:0", end_maxpool = False):
        ...
        if end_maxpool:
            self.add_layer = AddSNNLayer_ReLU()
        else:
...
class SpikingDense_positive_ReLU1(nn.Module):
    ...
    def __init__(self, units, input_units, name, weights, biases, X_n=1, outputLayer=False, robustness_params={}, input_dim=None,
                 kernel_regularizer=None, kernel_initializer=None):
        super(SpikingDense_positive_ReLU1, self).__init__()
        W = weights.T
        b1 = biases
        b2 = biases.detach().clone() - 1
        self.first = SpikingDense(units,"test",robustness_params=robustness_params)
        self.second = SpikingDense(units,"test",robustness_params=robustness_params)
...
class SpikingDense_pos_all(nn.Module):
    ...
    def __init__(self, units, input_units, name, weights, biases, X_n=1, outputLayer=False, robustness_params={}, input_dim=None,
                 kernel_regularizer=None, kernel_initializer=None):
        super(SpikingDense_pos_all, self).__init__()
        W1 = weights.T
        W2 = -weights.T
        W = torch.concatenate((W1,W2),dim=1)
